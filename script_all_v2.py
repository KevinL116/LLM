...
OUTPUT_DIR = "/mnt/data/LLM_outputs"  # Where generated Verilog files will be stored
RAW_OUTPUT_DIR = "/mnt/data/RAW_LLM_outputs"  # Folder to store original LLM responses
MAX_RETRIES = 5  # Number of retries on compilation failure
...
You should never include any additional text or comments before the code.
...
def extract_verilog_code(output):
    """
    Extract all Verilog modules from the LLM-generated output.
    This ensures extraction starts from 'module' and ends at 'endmodule',
    handling multiple module declarations correctly.
    """
    # Remove comment lines before extracting modules
    output = re.sub(r"^\s*//.*$", "", output, flags=re.MULTILINE)  # Remove `//` comments
    output = re.sub(r"/\*[\s\S]*?\*/", "", output)  # Remove `/* ... */` comments

    # Improved regex to capture everything from 'module' to 'endmodule'
    verilog_pattern = r"\bmodule\s+[\w\d_]+\s*[\s\S]*?endmodule\b"

    # Find all matches
    matches = re.findall(verilog_pattern, output, re.DOTALL)

    if matches:
        return "\n\n".join(matches)  # Join multiple modules with spacing
    else:
...
def generate_verilog(design_name, prompt_content, output_file, raw_output_file):
    ...
    if USE_API:
        ...
        # command = f'ollama run qwen2.5-coder:14b "{full_prompt}"'
